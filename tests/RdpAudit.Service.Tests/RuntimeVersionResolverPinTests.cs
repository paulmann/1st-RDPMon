...
//          1.3.0 — the SemVer publish.ps1 emits and the value the Configurator's Service tab
...
//          and the resolver path the running service actually uses at runtime. The 1.3.0 release
//          corresponds to the IPC reliability/diagnostics overhaul (per-command timeouts,
//          structured IpcCallResult, SCM-aware reachability, last-known-data UI, version/SHA
//          mismatch warning). If the release stream advances, this test must move with it.
...
/// <summary>Pins the Service runtime version at exactly 1.3.0, blocking both the prior 1.0.0
